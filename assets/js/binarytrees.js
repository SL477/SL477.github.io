...
class BST {
  ...
  insert(data) {
    const n = new NodeCls(data, null, null);
    if (this.root === null) {
      this.root = n;
    }
    else {
...
function startUp() {
  ...
  const grades = getArray(100);
  console.log('Print grade array:', prArray(grades));
  const gradeddistro = new BST();
  for (let i = 0; i < grades.length; ++i) {
    const g = grades[i];
    //console.log('g',g);
    const grade = gradeddistro.find(g);
    if (grade === null) {
...
function getTextCounts() {
  ...
  const txtCounter = new BST();

  textInput.split(' ').forEach(element => {
    const inp = txtCounter.find(element);
    if (inp === null) {
